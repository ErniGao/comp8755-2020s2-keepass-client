...
namespace KeePass.Forms
{
	...
	public partial class MainForm : Form, IMruExecuteHandler, IUIOperations
	{
        ...
        private Dictionary<int, string> configList;
		/// <summary>
		/// Default constructor.
		/// </summary>
		public MainForm()
		...
		private void OnFormLoad(object sender, EventArgs e)
		{
			...
			if(AppPolicy.Current.Plugins)
			{
				...
				ToolStripItemCollection tsicG = m_ctxGroupList.Items;
				int ciG = tsicG.Count;
				ToolStripItem tsiPrevG = ((ciG >= 1) ? tsicG[ciG - 1] : null);

				ToolStripItemCollection tsicE = m_ctxPwList.Items;
				int ciE = tsicE.Count;
				ToolStripItem tsiPrevE = ((ciE >= 1) ? tsicE[ciE - 1] : null);
				...
				m_pluginManager.AddMenuItems(PluginMenuType.Group, tsicG, tsiPrevG);
				m_pluginManager.AddMenuItems(PluginMenuType.Entry, tsicE, tsiPrevE);
		...
		public void SaveDatabase(PwDatabase pdToSave, object sender)
		{
            ...
            Console.WriteLine("not saving database as!!!!!!!!!!!!!");
		...
		private void OnPwListFind(object sender, EventArgs e)
		{
			...
			if(sf.ShowDialog() == DialogResult.OK)
			{
				...
				SearchParameters sp = sf.SearchResultParameters;

				UpdateEntryList(pgResults, false);
				SelectFirstEntryIfNoneSelected();

				UpdateUIState(false);
				ShowSearchResultsStatusMessage(sp.RespectEntrySearchingDisabled ?
					pgRoot : null);
		...
		private void OnEntryClipCopy(object sender, EventArgs e)
		{
			...
			try
			{
				EntryUtil.CopyEntriesToClipboard(pd, vSelected, this.Handle);
				StartClipboardCountdown();
			}
        ...
        /// <summary>
        /// turns our config data into a dictionary
        /// </summary>
        /// <param name="configFiles"></param>
        /// <returns>dictionary of database versions in shares and their locations</returns>
        private Dictionary<Shares, List<string>> GetShareVersions(string[] configFiles, out Dictionary<int, string> sharesPresent)
        {
            string[] split;
            string fileLocation;
            
            List<int> shares;
            sharesPresent = new Dictionary<int, string>();
            // this is to keep a list of shares in on location
            // this is read through the config file
            // We need to define a custom comparer to enable comparisons on the dictionary keys as custom objects
            Dictionary<Shares, string> localSharePairs = new Dictionary<Shares, string>(new MyComparer());
            Dictionary<Shares, List<string>> sharePairs = new Dictionary<Shares, List<string>>(new MyComparer());
            
            //loop through all the config files one at a time
            for (int i = 0; i < configFiles.Length; i++)
            {
                split = configFiles[i].Split(',');
                for (int j = 1; j < split.Length; j = j + 2)
                {
                    try
                    {
                        //get list of shares used in this version of the database
                        shares = split[j].ToCharArray().Select(x => Convert.ToInt32(x.ToString())).ToArray().ToList();
                        Shares s = new Shares(shares);

                        // get filename
                        fileLocation = split[j + 1];

                        //make list of shares that are present
                        if (!sharesPresent.ContainsKey(int.Parse(split[0])))
                            sharesPresent.Add(int.Parse(split[0]), Path.GetDirectoryName(fileLocation));

                        if (localSharePairs.ContainsKey(s))
                        {
                            localSharePairs[s] = fileLocation;
                        }
                        else
                        {
                            localSharePairs.Add(s, fileLocation);
                        }
                    }
                ...
                foreach (KeyValuePair<Shares, string> e in localSharePairs)
                {
                    if (sharePairs.ContainsKey(e.Key))
                    {
                        sharePairs[e.Key].Add(e.Value);
                    }
                    else
                    {
                        sharePairs.Add(e.Key, new List<string>( new string[] {e.Value }));
                    }
                }
                localSharePairs.Clear();
            }
            return sharePairs;
        }
        //compare Shares object by the list of shares
        private class MyComparer : IEqualityComparer<Shares>
        {
            public bool Equals(Shares a, Shares b)
            {
                return true;
            }
            public int GetHashCode(Shares obj)
            {
                int hash = 19;
                foreach (var foo in obj.GetShares())
                {
        ...
        /// <summary>
        /// Get combination of shares we need to merge
        /// </summary>
        /// <param name="shares"></param>
        /// <param name="n"></param>
        /// <param name="threshold"></param>
        /// <returns>returns a list of shares we need to merge</returns>
        private List<List<int>> GetCombinations(int n, int threshold)
        {
            List<int> elements = Enumerable.Range(1, n).ToList();
            
            List<List<int>> allCombinations = ItemCombinations<int>(elements, threshold, n);
            List<List<int>> combinations = new List<List<int>>();
            Boolean valid = true;
            foreach (List<int> e in allCombinations)
            {
                foreach (int s in elements)
                {
                    if (!e.Contains(s))
                        valid = false;
                }
        ...
        private void ShamirOpen(object sender, EventArgs e)
        {
            ShamirOpenForm openForm = new ShamirOpenForm();
            try
            {
                if (openForm.ShowDialog() == DialogResult.OK)
                {
                
                    byte[][] configContents = Array.ConvertAll<String, byte[]>(openForm.GetConfigFiles(), x => File.ReadAllBytes(x));

                    string[] pairs = Array.ConvertAll<byte[], String>(configContents, x => Encoding.UTF8.GetString(x));

                    string[] shareLocations;
                    string[] locations = openForm.GetConfigFiles();

                    Dictionary<Shares, List<string>> shareHistory = GetShareVersions(pairs, out Dictionary<int, string> sharesPresent);

                    // threshold scheme set to 5, 3
                    List<List<int>> ValidCombinations = GetCombinations(5, 3);
                    List<KeyValuePair<List<int>, List<string>>> sharesToMerge = new List<KeyValuePair<List<int>, List<string>>>();

                    //get all the sets of shares that can be used to reconstruct a database
                    foreach (KeyValuePair<Shares, List<string>> kvp in shareHistory)
                    {
                        if (kvp.Value.Count >= 3)
                        {
                            sharesToMerge.Add(new KeyValuePair<List<int>, List<string>>(kvp.Key.GetShares(), kvp.Value));
                        }
                    }

                    // alert user if not enough shares are supplied
                    if (!sharesToMerge.Any()) throw new InvalidOperationException();

                    //start by opening the first set of shares
                    shareLocations = sharesToMerge[0].Value.ToArray();
                    configList = sharesPresent;
                    int counter = 1;
                    List<string> dbLocations = new List<string>();
                    string dbPath = Path.GetTempPath() + counter++ + ".kdbx";
                    dbLocations.Add(dbPath);

                    openForm.Merge(shareLocations, dbPath);
                    OpenDatabase(null, null, true, openForm.GetPath());

                    //save the other shares into databases in the same folder
                    //then synchronise the shares that are present.
                    for (int i = 1; i < sharesToMerge.Count; i++)
                    {
                        shareLocations = sharesToMerge[i].Value.ToArray();
                        dbPath = Path.GetTempPath() + counter++ + ".kdbx";
                        dbLocations.Add(dbPath);
                        openForm.Merge(shareLocations, dbPath);
                    }

                    bool? b = ImportUtil.ShamirSynchronize(m_docMgr.ActiveDatabase, this, false, this, dbLocations);
                    UpdateUI(false, null, true, null, true, null, false);

                    try
                    {
                        if (File.Exists(openForm.GetPath()))
                        {
                            File.Delete(openForm.GetPath());
                        }
                    }
            ...
            // Alert user when the config file cannot be found
            catch (IOException)
            {
                MessageBox.Show("Please supply valid folders with config files in them",
                    "Invalid location", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            // Alert user when not enough shares are present
            catch (InvalidOperationException)
            {
                MessageBox.Show("The number of shares provided is less than the threshold!",
                    "Invalid number of shares provided", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void ShamirSave(object sender, EventArgs e)
        {
            string[] locations;
            ShamirQuickSaveForm shamirSplitForm = new ShamirQuickSaveForm();

            int threshold, count;
            string path;
            if (shamirSplitForm.ShowDialog() == DialogResult.OK)
            {
                threshold = shamirSplitForm.GetThreshold();
                count = shamirSplitForm.GetShares();
                path = Path.GetTempPath();
                ShareSave(null, null, false, sender, false, count, threshold, path, configList);
            }
        }

        private void ShamirSynchronizeAll(object sender, EventArgs e)
        {
            ShamirSaveAllForm shamirSync = new ShamirSaveAllForm();
            ...
            try
            {
                if (shamirSync.ShowDialog() == DialogResult.OK)
                {
                    threshold = shamirSync.GetThreshold();
                    count = shamirSync.GetShares();
                    path = Path.GetTempPath();
                    locations = shamirSync.GetLocations();
                    ShareSaveAs(null, null, false, sender, false, count, threshold, path, locations);
                }
            }
            catch (InvalidOperationException ex)
            {
                MessageBox.Show(ex.Message, "Invalid Splitting Scheme", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        /// <summary>
		...
		//start of modification by Erni Gao
		private void m_menuRemoteControl_Click(object sender, EventArgs e)
		{
			RemoteControlForm remoteControlForm = new RemoteControlForm();
			remoteControlForm.ShowDialog();
		}
		// end of modification by Erni Gao
